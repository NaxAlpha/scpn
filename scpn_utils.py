...
def indexify_transformations(in_p, out_p, label_voc, args):
    ...
    # otherwise, indexify and return
        ...
        for idx in range(len(in_seqs)):
            curr_in = in_seqs[idx]
            in_trans_np[idx, :len(curr_in)] = curr_in#[label_voc[z] for z in curr_in if z!='EOP']
            in_lengths.append(len(curr_in))

            curr_out = out_seqs[idx]
            out_trans_np[idx, :len(curr_out)] = curr_out#[label_voc[z] for z in curr_out if z!='EOP']
